...
class _StaffScreenState extends State<StaffScreen> {
  ...
  Future<void> _payAll() async {
    final total = _staff
        ...
        .fold<num>(0, (sum, s) => sum + (s['monthlySalary'] ?? 0));
    ...
    try {
      ...
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Paid ${result['staffCount']} staff!'),
            backgroundColor: AppTheme.accent,
          ),
        );
    } catch (_) {}
  ...
  Widget _payRow(String l, String r, {bool bold = false, Color? color}) => Row(
  ...
  Widget build(BuildContext context) {
    ...
    final walletBalance = auth.user?['business']?['walletBalance'] ?? 0;
                  ...
                  Padding(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        GestureDetector(
                          onTap: () async {
                            if (plan == 'GROWTH' && _staff.length >= 50) {
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(
                                  content: Text(
                                    'Maximum limit of 50 staffs reached. Please upgrade to the Scale plan.',
                                  ),
                                ),
                              );
                              return;
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          gradient: const LinearGradient(
                            colors: [AppTheme.yellow, Color(0xFFD97706)],
                          ),
                          borderRadius: BorderRadius.circular(16),
                        ),
                        child: Row(
                          ...
                          children: [
                            ...
                            Text(
                              'Pay All Workers — ${_fmt(_staff.where((s) => s['status'] == 'ACTIVE').fold<num>(0, (sum, s) => sum + (s['monthlySalary'] ?? 0)))}',
                              style: const TextStyle(
                                color: AppTheme.bgPrimary,
                                fontWeight: FontWeight.w700,
                                fontSize: 14,
