...
export class PaymentsService {
  ...
  async verifyWalletFunding(reference: string, userId: string) {
    ...
    if (!businessId)
      throw new HttpException('No business found', HttpStatus.BAD_REQUEST);
  ...
  async flutterwaveInitialize(
    ...
    const res = await fetch('https://api.flutterwave.com/v3/payments', {
      ...
      body: JSON.stringify({
        ...
        customizations: {
          title: 'BizhubNg Payment',
          description: description || 'Business payment',
        },
      }),
    });
    const data = await res.json();
    if (data.status !== 'success')
      throw new HttpException(
        data.message || 'Flutterwave error',
        HttpStatus.BAD_REQUEST,
      );
  ...
  async moniepointCreateVirtualAccount(
    userId: string,
    name: string,
    email: string,
  ) {
